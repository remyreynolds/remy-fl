...
Always emit MIDI as structured JSON the app converts to .mid:
...
```json
{
  ...
  "tracks": [
    {
      "name": "Chords", "channel": 0, "role": "chords",
      "notes": [
        {"pitch": 53, "start_tick": 0, "length_ticks": 420, "velocity": 92}
      ]
    }
  ]
}
...
- `start_tick` absolute from loop start; swing/jitter already baked into ticks
- One track per element; roles: `chords | bass | melody | arp | perc`
- Perc uses GM drum mapping (36 kick, 38 snare, 42 CH, 46 OH) on channel 9
- pitch is MIDI note number 21–108; velocity 1–127; length_ticks > 0
