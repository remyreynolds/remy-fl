                                   ...
                                   PatternCallback callback)
{
    ...
    if (! houseBrain.isLoaded())
        (void) houseBrain.loadFromDefaultLocations();
    const auto brainRoute = houseBrain.retrieveForGeneration (prompt, 16000);
    ...
    const auto brainRules = houseBrain.hardRules();

    auto retrieval = knowledgeBase.retrieveForQuery (prompt, 8000, true); // secondary guides
    ...
    juce::Thread::launch ([this, prompt, lockKey, brainCtx, brainArch, brainTitles, brainChunks,
                           brainRules, knowledgeCtx, projCtx, matchedDocs, docsUsed,
                           recentProgressions, recentFpList, variationNonce, ep, masterBrain, callback]
    {
        PatternResponse resp;
        ...
        // 1) PDF corpus sits ABOVE all other knowledge (user requirement).
        ...
        // Secondary knowledge (non-PDF guides) — below PDF in priority
        ...
        for (int attempt = 0; attempt < kMaxAttempts; ++attempt)
        {
            ...
            if (attempt > 0)
            {
                attemptUser << "\n\nREJECTED CANDIDATE (attempt " << juce::String (attempt + 1)
                            ...
                            << "\nRegenerate with a materially different musical solution that "
                               "obeys PDF hard rules (monophony for bass, density budgets, "
                               "originality thresholds, pipeline order).";
            }

            auto http = postChatCompletion (ep, system, attemptUser, 8192);
            if (! http.ok)
            {
                resp.ok = false;
                resp.error = http.error;
            ...
            // Existing harmony-memory reject (ch.9-adjacent)
            if (! rememberProgressionIfFresh (parsed.pattern) && attempt + 1 < kMaxAttempts)
            {
